...
export default function HomePage(){
 ...
 return <div className="thanks-page"><BrandHeader active="thanks" locale={language} languageControl={<BrandLanguage locale={language} onChange={setLanguage}/>}/><main className="thanks-scene"><section className="thanks-title" aria-labelledby="thanks-heading"><p>{copy.eyebrow}</p><h1 id="thanks-heading">{copy.title}</h1></section><div className="thanks-field" aria-label={copy.contributors}>{contributors.map(contributor=><FloatingContributor key={contributor.name} contributor={contributor} profile={copy.profile}/>)}</div></main><BrandFooter/></div>;
}
