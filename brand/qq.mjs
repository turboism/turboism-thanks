/** Shared QQ join UI for React pages and static SDK HTML. Native launch is available only through the explicit action.
...
const QQ_TEXT = {
  ko: { label:'Turboism QQ 그룹 참여', title:'함께 나누는 창작 이야기', subtitle:'Turboism QQ 커뮤니티', copy:'그룹 번호 복사', open:'QQ에서 열기', hint:'앱이 열리지 않으면 QQ에서 이 그룹 번호를 검색하여 가입을 신청하세요', copied:'그룹 번호를 복사했습니다', failed:'그룹 번호를 선택하여 직접 복사해 주세요', close:'닫기' },
  zh: { label:'加入 Turboism QQ 群', title:'一起聊聊创作', subtitle:'Turboism QQ 交流群', copy:'复制群号', open:'打开 QQ 加群', hint:'未能打开？在 QQ 中搜索群号，申请加入即可', copied:'群号已复制', failed:'请选中群号，手动复制', close:'关闭' },
  en: { label:'Join the Turboism QQ group', title:'A place to create together', subtitle:'Turboism community on QQ', copy:'Copy ID', open:'Open in QQ', hint:'No app opened? Search this group ID in QQ and request to join', copied:'Group ID copied', failed:'Select the group ID and copy it manually', close:'Close' },
  ja: { label:'Turboism の QQ グループに参加', title:'創作の話を、一緒に', subtitle:'Turboism QQ コミュニティ', copy:'番号をコピー', open:'QQ でグループを開く', hint:'開かない場合は QQ でこの番号を検索し、参加を申請してください', copied:'グループ番号をコピーしました', failed:'番号を選択して手動でコピーしてください', close:'閉じる' }
};
const QQ_CSS = `.tb-qq-host,.tb-qq{display:inline-flex;position:relative;flex-shrink:0}.tb-qq-trigger{border:0;background:none;cursor:pointer}.tb-qq-trigger:disabled{cursor:default}.tb-qq-trigger[aria-expanded=true]{color:var(--tb-violet);background:var(--tb-soft)}.tb-qq-panel[hidden]{display:none!important}.tb-qq-panel{position:absolute;right:0;top:calc(100% + 16px);width:330px;max-height:calc(100dvh - var(--tb-header-height) - 32px);overflow:auto;padding:23px;background:var(--tb-paper);color:var(--tb-ink);border:1px solid var(--tb-border);border-radius:12px;box-shadow:0 16px 48px #30294b19;text-align:left;font-family:var(--tb-sans);z-index:80;animation:tb-qq-in .16s ease-out}.tb-qq-top{display:flex;align-items:center;justify-content:space-between;gap:12px}.tb-qq-eyebrow{font-size:10px;letter-spacing:.14em;color:var(--tb-violet)}.tb-qq-close{display:grid;place-items:center;width:36px;height:36px;margin:-8px -9px -4px 0;background:none;border:0;border-radius:5px;color:var(--tb-muted);cursor:pointer}.tb-qq-close:hover{background:var(--tb-soft);color:var(--tb-violet)}.tb-qq-close svg{width:17px;height:17px;fill:none;stroke:currentColor;stroke-width:1.5}.tb-qq-panel h2{font:400 23px/1.5 var(--tb-display);letter-spacing:-.02em;margin:15px 0 5px;color:var(--tb-violet)}.tb-qq-subtitle{font-size:12px;color:var(--tb-muted);margin:0 0 20px;line-height:1.6}.tb-qq-number{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:7px 11px 7px 14px;background:var(--tb-soft);border-radius:6px}.tb-qq-number code{font:500 19px/1.5 var(--font-mono,monospace);letter-spacing:.06em;color:var(--tb-ink);user-select:all}.tb-qq-copy{font:500 11px/1.3 var(--tb-sans);min-height:40px;padding:8px;background:none;color:var(--tb-violet);border:0;cursor:pointer;border-radius:4px}.tb-qq-copy:hover{background:var(--tb-yellow)}.tb-qq-panel .tb-qq-open{display:flex;align-items:center;justify-content:space-between;gap:14px;min-height:46px;margin-top:16px;padding:12px 15px;background:var(--tb-violet);color:white;border-radius:6px;text-decoration:none;font-size:12px}.tb-qq-panel .tb-qq-open:hover{background:var(--tb-deep)}.tb-qq-hint{font-size:11px;line-height:1.85;color:var(--tb-muted);margin:15px 0 0}.tb-qq-status:empty{display:none}.tb-qq-status{font-size:11px;line-height:1.6;color:var(--tb-violet);min-height:18px;margin:8px 0 0}@keyframes tb-qq-in{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:translateY(0)}}@media(max-width:700px){.tb-qq-panel{position:fixed;top:calc(var(--tb-header-height) + 9px);left:16px;right:16px;width:auto;max-width:400px;margin-left:auto;padding:22px}}@media(prefers-reduced-motion:reduce){.tb-qq-panel{animation:none}}`;
...
export function attachQQ(root) {
  ...
  const updateLink = () => { launch.href = qqLink(win.navigator.userAgent); };
  ...
  const click = event => {
    ...
    status.textContent = ''; panel.focus({preventScroll:true});
    // Opening the panel never requests a native URI; only the separate link can.
  };
  ...
  const onLaunch = () => { updateLink(); status.textContent = ''; };
  const onCopy = async () => {
  ...
  localize(); updateLink(); trigger.disabled = false;
  return () => { disposed = true; trigger.disabled = true; observer.disconnect(); trigger.removeEventListener('click', click); launch.removeEventListener('click', onLaunch); copy.removeEventListener('click', onCopy); close.removeEventListener('click', onClose); doc.removeEventListener('pointerdown', outside); doc.removeEventListener('keydown', key); node.removeEventListener('focusout', focusOut); };
}
